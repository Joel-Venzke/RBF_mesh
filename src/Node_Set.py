...
class Node_Set:
    ...
    def get_poly_terms(self, nodes, poly_order):
        ...
        if current_dim == 1:
            ...
            current_dim_poly = self.get_poly_terms(nodes[:,-1].reshape((nodes.shape[0],1)), poly_order)
            ...
            num_poly_terms = comb(poly_order+current_dim,current_dim,exact=True)
            # allocate array
            poly_set = np.zeros((nodes.shape[0],num_poly_terms))
            # populate new array
            col = 0
            for poly_deg in range(0,poly_order+2): # loop over all poly degrees
                for cur_degree in range(0,poly_deg): # loop over poly degrees
                    # find upper and lower bound for dim-1 poly list
                    lower_dim_poly_upper_idx = comb((poly_deg-cur_degree-1)+current_dim-1,current_dim-1,exact=True)
                    lower_dim_poly_lower_idx = comb((poly_deg-cur_degree-2)+current_dim-1,current_dim-1,exact=True)
                    # find number of poly terms to multiply together
                    col_offset = lower_dim_poly_upper_idx - lower_dim_poly_lower_idx
                    # calculate terms
                    poly_set[:,col:col+col_offset] = lower_dim_poly[:,lower_dim_poly_lower_idx:lower_dim_poly_upper_idx] * current_dim_poly[:,cur_degree].reshape((current_dim_poly.shape[0],1))
    ...
    def get_average_spacing(self, node_set):
        nearest_dist, nearest_idx = self.get_nearest_neighbors_from_set(node_set, 2)
    ...
    def create_node_set(self):
        ...
        print "Match radius:", match_radius, self.get_average_spacing(shell_nodes*match_radius)
        self.node_set = []
    ...
    def calculate_opperator_weights(self):
        ...
        for idx, node_list_idx in enumerate(self.nearest_idx):
            ...
            for node_idx, row_node in enumerate(node_list_shifted):
                A_matrix[node_idx, :self.stencil_size] = self.rbf_phi(np.sqrt(((node_list_shifted-row_node)**2).sum(axis=1)))
                right_hand_side[node_idx] = self.rbf_order*(self.rbf_order+3.0-2.0)*np.sqrt(((row_node)**2).sum())**(self.rbf_order-2)
            ...
            A_matrix[:self.stencil_size, self.stencil_size:self.stencil_size+num_poly_terms] = self.get_poly_terms(node_list_shifted, self.poly_order)
            A_matrix[self.stencil_size:self.stencil_size+num_poly_terms, :self.stencil_size] = A_matrix[:self.stencil_size, self.stencil_size:self.stencil_size+num_poly_terms].transpose()

            # add exp (use unshifted distance)
            for a in np.arange(1,self.exp_order+1):
                node_list_radius = np.sqrt(((node_list)**2).sum(axis=1))
                right_hand_side[matrix_size - self.exp_order - 1 + a
                    ] = a*a*np.exp(-a*node_list_radius[0]
                        ) - 2.0*a/node_list_radius[0]
                exp_row = np.exp(-a*node_list_radius)
                A_matrix[:self.stencil_size,
                         matrix_size - self.exp_order-1+a] = exp_row
